...
export class Stepper implements OnDestroy {
    ...
    bindStepper(stepper: any, opts?: any) {
        const self = this, onchange = (opts) ? opts.onchange : function () { };
        // Bind field
    ...
    handleKeyPress(e: KeyboardEvent) {
        ...
        const charCode = ('charCode' in e ? e.charCode : e.keyCode);
        ...
        if (charCode === 45 && (!(typeof this.min !== 'undefined') || this.min < 0)) { return; }
    ...
    start(e: Event, up: boolean) {
        ...
        // set the update step
        const step = (up ? (this.getNum(this.stepper.field.step)) : -(this.getNum(this.stepper.field.step)));
    ...
    private updateStepperCount(amount: number, stepper: any) {
        ...
        // reduce the delay
        this.timeoutDelay = 500;
        // Math.max(20, Math.floor(this.timeoutDelay * 0.9));
