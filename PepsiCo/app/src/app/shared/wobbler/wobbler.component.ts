...
export class WobblerComponent implements OnInit {
    ...
    isGoodWobblerLocation() {
        ...
        let goodLocation = ((currentRoute != "/intro") &&
            // (currentRoute != "/planningtool") &&
            (currentRoute.indexOf("/planningtool") == -1));
    ...
    onSubmitWobbler() {
        if (this.answer != -1 || this.readOnly) {
            ...
            this.wobbler["wobblerCode"] = this.wobbler["type"] == "InfoUpdate" ? 1 : "W1" + this.wobbler["alt" + this.answer].toLowerCase();
    ...
    isWobblerViewed() {
        let isAllWebblerChecked = 0;
        if (this.wobblersArray.length > 0) {
            for (let i = 0; i < this.wobblersArray.length; i++) {
                if (this.wobblersArray[i]["isViewed"])
                    ++isAllWebblerChecked;
            }
            if (isAllWebblerChecked != this.wobblersArray.length)
                return false;
            else
                return true;
        }
