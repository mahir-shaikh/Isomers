...
@Directive({
    selector: '[ismaNumberFormatter]',
    providers: [NumberFormattingPipe]
})
export class CalcFormatterDirective implements OnInit, AfterViewInit {
    ...
    onFocus(value: string) {
        ...
        if (!this.inputFormatted) {
            this.el.value = this.numberFormatter.parse(value, this.scaler) as any;
    ...
    onBlur(value: string) {
        ...
        this.el.value = this.numberFormatter.transform(value, this.format, this.scaler) as any;
