  ...
  return content.replace(/styleUrls\s*:\s*(\[[\s\S]*?\])/gm, function (m, styleUrls) {
    ...
    return 'styles: ['
      + urls.map(styleUrl => {
        ...
        const styleContent = styleFile.endsWith('.scss') ? buildSass(originContent, styleFile) : styleFile.endsWith('.styl') ? buildStyl(originContent, styleFile) :  originContent;
...
/**
 * build styl content to css
 * @param content {string} the css content
 * @param sourceFile {string} the styl file sourceFile
 * @return {string} the generated css, empty string if error occured
 */
function buildStyl(content, sourceFile) {
  try {
    /* {
      data: content,
      file: sourceFile,
    ...
    const result = stylus(content).set('filename', sourceFile).render();
    console.log(result)
