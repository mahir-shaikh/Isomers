...
//				var tmpNum = Number((this.num(curCtx)).toPrecision(10));
...
//        curSum = Number(curSum.toPrecision(10));
...
//        console.log ("NOT: " + Bval + ", " + !Bval + " --- " + Vval + ", " + !Vval);
	...
	WorksheetFunctions.VLOOKUP = function(argArray, ctx) {
        ...
        // the default value for the Match (4th) parameter is approximate, which is TRUE in Excel
        // this seems counter-intuitive, since a value of TRUE would seem to indicate an exact match
        // however, here TRUE indicates an approximate match, thus the default value below of FALSE for the exact match variable
		var exact = false;
...
//		}
    ...
    WorksheetFunctions.LEFT = function(argArray, ctx) {
        var text = argArray[0](ctx).s(ctx),
            num = (argArray[1] && argArray[1](ctx) && argArray[1](ctx).num(ctx)) ? argArray[1](ctx).num(ctx) : 1;
    ...
    WorksheetFunctions.SUBSTITUTE = function(argArray, ctx) {
    	var text = argArray[0](ctx).s(ctx),
    		...
    		occurence = (argArray[3]) ? argArray[3](ctx).num(ctx) : undefined;
