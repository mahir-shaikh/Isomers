...
export class CalcDropdown implements OnInit, OnDestroy, OnChanges {
    ...
    dropdownChanged($item: string): void {
        ...
        if (this.format && $item != this.placeholder) {
    ...
    ngOnInit() {
        ...
        if (this.yearRef) {
            ...
            let val = this.calcService.getValue(this.ref, true);
            if(val != this.placeholder){
                modelVal = this.numberFormatting.transform(val, this.format, this.scaler);
            }else{
                modelVal = val;
            }
        }
        ...
        this.observer = this.calcService.getObservable().subscribe(() => {
            ...
            if (self.yearRef) {
                ...
                let val = self.calcService.getValue(self.ref, true);
                if(val != self.placeholder){
                    modelVal = self.numberFormatting.transform(val, self.format, self.scaler);
                }else{
                    modelVal = val;
                }
            }
