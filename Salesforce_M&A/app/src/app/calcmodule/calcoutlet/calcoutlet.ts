...
export class CalcOutlet {
    ...
    private updateValue() {
        ...
        this.newValue = this.calcService.getValueForYear(this.ref, this.yearRef, this.useRawValue);
        // if (self.value == self.newValue) return;
        if(this.showBalloon){
        ...
        if(this.highlightNegative){
            if( this.isProper(this.value) && this.numberFormatting.transform(this.value,this.format,this.scaler).indexOf("(") != -1){
    ...
    isProper(number): boolean {
        if((this.value) && this.value != "" && this.value != "NaN" && this.value.indexOf("#") == -1 && this.numberFormatting.transform(this.value,this.format,this.scaler) ){
            return true;
        }else{
            return false;
        }
    ...
    calculateDifference(){
        let curValue = this.numberFormatting.parse(this.newValue),
            ...
            difference;
        if(curValue > prevValue && ((curValue - prevValue) > 0.001)){
        ...
        }else if(curValue < prevValue && ((prevValue - curValue) > 0.001)){
