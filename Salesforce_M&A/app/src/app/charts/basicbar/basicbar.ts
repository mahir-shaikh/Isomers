import { Component, Input, ViewChild, Directive, ElementRef, OnInit, OnDestroy, EventEmitter, SimpleChanges, HostListener } from '@angular/core';
...
export class BasicBarComponent implements OnDestroy, OnInit {
    ...
    @Input() forceRedraw: boolean = false;
    ...
    onModelChange(){
        ...
        for (let i=0; i<self.localCategoryLabelsArray.length; i++) {
            // self.chartData.xAxis.categories[i] = self.textengineService.getText(self.localCategoryLabelsArray[i]);
            if ((self.localCategoryLabelsArray[i].indexOf("tlInput") != -1) || (self.localCategoryLabelsArray[i].indexOf("tlOutput") != -1) || (self.localCategoryLabelsArray[i].indexOf("glo") != -1)) {
                self.chart.xAxis[0].categories[i] = self.calcService.getValue(self.localCategoryLabelsArray[i]);
            } else {
                self.chart.xAxis[0].categories[i] = self.textengineService.getText(self.localCategoryLabelsArray[i]) || self.localCategoryLabelsArray[i];
            }
        }
