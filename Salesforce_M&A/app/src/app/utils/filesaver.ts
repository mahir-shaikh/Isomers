...
export class FileSaver {
    ...
    fileSaveAs(fileName:string, jsonOb:any, delim?:string, lineEnding?:string, isJsonCombined: boolean = false, seprator?:string) {
        ...
        var csvString = isJsonCombined ? this.generateSeperateStringDataFromCombinedJSON(jsonOb, delim, lineEnding,seprator) : this.generateStringDataFromJSON(jsonOb, delim, lineEnding)
        ...
        if (navigator.msSaveBlob) { // IE10+ : (has Blob, but not a[download] or URL) 
            window.navigator.msSaveOrOpenBlob(dataURI, fileName)
            //return navigator.msSaveBlob(dataURI, fileName); 
        } 
    ...
    private generateSeperateStringDataFromCombinedJSON(jsonOb, delimChar, lineEnding, separator) {
        var header
            , DELIMITER = (typeof delimChar !== "undefined" && delimChar) ? delimChar : String.fromCharCode(59) /* defaults to ';' */
            , rows = ''
            , NEW_LINE = (typeof lineEnding !== "undefined" && lineEnding) ? lineEnding : '\n'
            , SEPERATER = (typeof separator !== "undefined" && separator) ? separator : '_';

        header = "RangeName" + DELIMITER + "RangeValue";

        Object.keys(jsonOb).forEach((key) => {
            let content = this.escapeQuotes(jsonOb[key]).split(SEPERATER);
            rows += key + DELIMITER + content + NEW_LINE;
        });
