define([], function() {
		...
		switch(typeof(value)) {
			case 'string':
					if(value.toUpperCase() == 'TRUE' || value.toUpperCase() == 'FALSE') {
						...
						return val;
					} else if (new RegExp(/[a-z|\x00-\x7F]+/i).test(value)) {
	...
	*/
	jsCalcValue.ReferenceValue = function(startRow, startCol, endRow, endCol, cellCtx, sheetName) {
		...
		val = new jsCalcValue.jsCalcReferenceValue(startRow, startCol, endRow, endCol, cellCtx, sheetName);
...
*/
	...
	*/
	jsCalcValue.jsCalcReferenceValue.prototype.getReference = function(ctx) {
		...
		} else if(ctx.col >= this.startCol && ctx.col <= this.endCol && ctx.row >= this.startRow && ctx.row <= this.endRow) {
			return this.targetSheetOb.getCell(ctx.row, ctx.col);
		} else {
	...
	*/
	jsCalcValue.jsCalcReferenceValue.prototype.eachCell = function(func, ctx) {
		...
		for(var r = this.startRow; r <= this.endRow; r++) {
			for(var c = this.startCol; c <= this.endCol; c++) {
				func.call(this.targetSheetOb.getCell(r, c));
			}
		}
	}
	...
	*/
	...
	jsCalcValue.jsCalcReferenceValue.colIndex = function(col, ctx) {
	...
	*/	
	...
	jsCalcValue.jsCalcStringValue.prototype.num = function(ctx) {
		return this.data.length>0?parseFloat(this.data):0;
	}
	...
	jsCalcValue.jsCalcErrorValue.name = "#NAME!";
