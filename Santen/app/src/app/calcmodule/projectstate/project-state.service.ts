...
export class ProjectStateService {
    ...
    deleteProject(projectIdentifier: string): Promise<any> {
        return new Promise((resolve, reject) => {
            if (!(this.projectExists(projectIdentifier))) {
                reject("Project does not exist!");
                return;
            }
            this.calcService.deleteProjectState(projectIdentifier).then(() => {
                this.projects.remove(projectIdentifier);
                this.dataService.setValue(PROJECT_KEYS, this.utils.stringifyDictionary(this.projects)).then(resolve);
            }, reject);
        });
    }
}
