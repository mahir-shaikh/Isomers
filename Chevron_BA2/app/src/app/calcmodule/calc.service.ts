...
export class CalcService {
    ...
    private localStorageKey: string = "modelstate:ChevronBA2";
    ...
    initialize(model: string, customActions: string, loadingProgressCb:Function = null): Promise<this> {
        ...
        this.modelChangeObserver = self.modelChangeEvent.auditTime(5000).subscribe(() => {
   ...
   private exportDecisions(isSpecificExport?:boolean, specificRangeNames?:any) {
        ...
        var teamYear = this.getValue("xxYear");
        var fileNamePrefix = "Chevron_InputData";
    ...
    runCourseAction(actionName:string): Promise<any> {
        return this.api.runCourseAction(actionName);
    }

    saveScenario(arrInputRefs:Array<string>, arrOutputRefs:Array<string>): Promise<any> {
        return new Promise((resolve, reject) => {
            arrInputRefs.forEach((ref, refIndex) => {
                let currentVal = this.getValue(ref, true);
                this.setValue(arrOutputRefs[refIndex], currentVal);
            })
            resolve();
        })
    }

    clearScenario(arrOutputRefs:Array<string>): Promise<any> {
        return new Promise((resolve, reject) => {
            arrOutputRefs.forEach((ref, refIndex) => {
                this.setValue(ref, "");
            })
            resolve();
        })
