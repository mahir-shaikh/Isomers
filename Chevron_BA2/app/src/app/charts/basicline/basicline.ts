    ...
    ngOnInit() {
        ...
        self.calcService.getApi().then((api) => {
            ...
            for (let i = 0; i < this.localSeriesLabelsArray.length; i++) {
                ...
                let localData = null;
                seriesPoints = [];
                for (let k = 0; k < self.rangeref[i].length; k++) {
                    points = [];
                    if (i == 0 || i == 1) {
                        // if (i != self.localCategoryLabelsArray.length - 1) {
                        for (let l = 0; l < self.rangeref[i][k].length; l++) {
                            localData = self.calcService.getValueForYear(self.rangeref[i][k][l], self.year, true);
                            Math.round(localData);
                            points.push(localData);
                        }
                        localDataArray.push(points);
                    }
                    else {
                        localData = self.calcService.getValueForYear(self.rangeref[i][k], self.year, true);
                        seriesPoints.push(localData);
                    }
                }
                ...
                // }
                localDataArray.push(seriesPoints);
                //}
                ...
                if (localSeriesName.indexOf("Scenario") != -1) {
            ...
            self.observer = api.getObservable().subscribe(() => {
                ...
                for (let i = 0; i < this.localSeriesLabelsArray.length; i++) {
                    ...
                    let localData = null;
                    seriesPoints = [];
                    for (let k = 0; k < this.rangeref[i].length; k++) {
                        points = [];
                        if (i == 0 || i == 1) {
                            // if (i != this.localCategoryLabelsArray.length - 1) {
                            for (let l = 0; l < this.rangeref[i][k].length; l++) {
                                localData = this.calcService.getValueForYear(this.rangeref[i][k][l], this.year, true);
                                Math.round(localData);
                                points.push(localData);
                            }
                            localDataArray.push(points);
                            ...
                            localData = this.calcService.getValueForYear(this.rangeref[i][k], this.year, true);
                            seriesPoints.push(localData);
                        }
                    }
                    // for (let j = 0; j < this.localCategoryLabelsArray.length; j++) {
