...
export class ChartDefaults {
    ...
    getDefaults():any {
        ...
        let defaults = {
            chart: {
                ...
                events : {
                    beforePrint: function () {
                        this.title.styles.visibility = "visible";
                        this.setTitle(this.title.textStr,null,true);
                    },
                    afterPrint : function() {
                        this.title.styles.visibility = "hidden";
                        this.setTitle(this.title.textStr,null,true);
                    }
                }
            },
            ...
            colors: ['#28324B','#FB054B', '#00AEEF', '#E61C3A', '#7F7F7F', '#C32B3D', '#ECF0F1', '#2C3E50', '#bcf08d', '#a65c34', '#ff4261', '#45dbcb', '#69261b', '#ff5050'],
            credits: {
                enabled: false
            },
            xAxis: {
    ...
    getStackedColumnChartDefaults(numberFormat?:string,totalName?:string,totalValue?:number) {
        let chartData = {
            chart: {
                type: 'column'
            },
            ...
            yAxis: {
                ...
                stackLabels: {
                    ...
                    formatter: function() {
                        if (numberFormat) {
                           return totalName +": " + numberFormatting.format(totalValue, numberFormat);
                        }
                        else {
                            return this.total;
                        }
            ...
            tooltip: {
                ...
                pointFormatter: function() {
                    if (numberFormat) {
                        return this.series.name + ": " + numberFormatting.format(this.y, numberFormat) + "<br />" + totalName +": " + numberFormatting.format(totalValue, numberFormat);
                    }
                    else {
                        return this.series.name+ ": " + this.y + "<br/>"+totalName+": " + this.stackTotal;
                    }
                }
            },
            plotOptions: {
    ...
    getBasicLineChartDefaults(numberFormat?:string) {
        let chartData: any = {
            ...
            tooltip: {
                // headerFormat: '<div> NPV: {point.y} <br>PV Investment: {point.x}</b> </div>',
                // pointFormatter: function() {
                //     debugger;
                //     if (numberFormat) {
                //         return numberFormatting.format(this.y, numberFormat);
                ...
                formatter: function () {
                    let valX, valY;
                    if (numberFormat) {
                        valX = numberFormatting.format(this.x, numberFormat);
                        valY = numberFormatting.format(this.y, numberFormat);
                    }
                    else {
                       valX = this.x;
                       valY = this.y;
                    }
                    return '<div> NPV: '+valY+' <br>PV Investment: '+valX+'</b> </div>';
                }
            },
            series: []
        };
