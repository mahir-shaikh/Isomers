...
export class SplashComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        Promise.all([calcModelLoadedPromise, textLoadedPromise/*, projectServiceLoaded*/,emailSavedPromise]).then(() => {
                stateUrl = (this.calcService.getStateUrl() && this.calcService.getStateUrl() !== "") ? this.calcService.getStateUrl() : '/opsOperation';
                            ...
                            if (erData !== null) {
                                erData.then((ready) => {
                                    if(ready == "true"){
                                        ...
                                        self.router.navigateByUrl('/erPlatform');
                                    }
                                });
                            }
                            else {
                                this.router.navigateByUrl('/erPlatform');
                            }
                        }
                        else {
