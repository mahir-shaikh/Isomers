...
export class StackedColumnComponent implements OnDestroy, OnInit, OnChanges {
    ...
    InitializeModel(){
        ...
        this.chartData = this.chartDefaults.getStackedColumnChartDefaults(this.numberformat);
        ...
        try {
            ...
            for (let i=0; i<self.localSeriesLabelsArray.length; i++) {
                ...
                let dataLabel;
                if(this.dataLabelsArray.length > 0){
                    dataLabel = this.calcService.getValueForYear(this.dataLabelsArray[i], self.year);
                }
    ...
    processComponentInputs() {
        ...
        if (dataLabels != undefined) {
            dataLabels= dataLabels.toString();
            dataLabels = dataLabels.replace(/\[|\]/g, '');
            dataLabels = dataLabels.replace(/ /g, '');
            this.dataLabelsArray = dataLabels.split(",");
        }
