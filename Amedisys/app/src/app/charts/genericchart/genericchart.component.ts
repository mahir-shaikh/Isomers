...
export class GenericChartComponent implements OnDestroy, OnInit {
    ...
    ngOnInit() {
        ...
        chartDefaults.chart.width = self.width;
        chartDefaults.chart.height = self.height;
                ...
                tooltip: {
                    pointFormatter: function() {
                        if (self.numberFormat) {
                            return this.series.name + ": " + numberFormatting.format(this.y, self.numberFormat);
                        }
                        else {
                            return this.y;
                        }
        ...
        if (this.chartoptions !== "" && this.chartoptions !== undefined) {
        ...
        this.observer = this.calcService.getObservable().subscribe(() => {
            ...
            if (this.type === CHART_TYPE.COMBINATION) {
                this.seriesOptions.forEach((series: Series, seriesIndex) => {
                    let _series={};
                    _series["data"] = this.fetchSeriesData(series.data);

                    arrSeriesData.push(_series["data"]);
                });
            }else{
                arrSeriesData = this.fetchDataFromModel();
            }
    ...
    processCategoryLabels() {
        for (let i = 0; i < this.localCategoryLabelsArray.length; i++) {
            if ((this.localCategoryLabelsArray[i].indexOf("tlInput") != -1) || (this.localCategoryLabelsArray[i].indexOf("tlOutput") != -1)|| (this.localCategoryLabelsArray[i].indexOf("glo") != -1)) {
    ...
    processSeriesOptions() {
        ...
        seriesOptions.forEach((series: Series, seriesIndex) => {
            ...
            // _series.name = 
            if ((series.name.indexOf("tlInput") != -1) || (series.name.indexOf("tlOutput") != -1)|| (series.name.indexOf("glo") != -1)) {
                    _series["name"] = this.calcService.getValue(series.name);
            } else {
                _series["name"] = this.textEngineService.getText(series.name) || series.name;
            }
            _series["data"] = this.fetchSeriesData(series.data);
            _series["showInLegend"] = self.showInLegend.indexOf(series.name) != -1 ? true : false;
    ...
    fetchSeriesData(dataRefs:Array<string>): Array<Number> {
        ...
        dataRefs.forEach((ref) => {
            let value = this.calcService.getValueForYear(ref, this.year, true),
