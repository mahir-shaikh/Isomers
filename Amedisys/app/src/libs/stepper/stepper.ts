...
export class Stepper implements OnDestroy{
    ...
    private updateStepperCount(amount:number, stepper:any) {
        ...
        // reduce the delay
        this.timeoutDelay = 500;//Math.max(20, Math.floor(this.timeoutDelay * 0.9));
    ...
    private onClick(amount:number, stepper:any, onChange?:Function) {
        ...
        if (!stepper.validator(amount)){
            // if(amount < 0){
            // we can't rely on the clicked button because of the ability to directly enter (potentially invalid) numbers, then click the stepper button
            if (this.getNum(stepper.field.value) <= this.getNum(stepper.field.min)) {
                amount = this.min;
            } else {
                amount = this.max;
            }
        ...
        if (this.getNum(stepper.field.value) < this.getNum(stepper.field.min) || this.getNum(stepper.field.value) > this.getNum(stepper.field.max)){
            // we can't rely on the clicked button because of the ability to directly enter (potentially invalid) numbers, then click the stepper button
            // if(amount < 0){
            if (this.getNum(stepper.field.value) <= this.getNum(stepper.field.min)) {
                amount = this.min;
