define(['../numeral/numeral', '../moment/moment'], function (numeral, moment) {
    ...
    numberFormatting.unformat = function (value, scaler) {
        //fix scaler = 0 and scaler not provided
        scaler = scaler ? scaler : 1;
        // numeral unformatted value is 0, but direct conversion to number fails - maybe encountering a string value so send ahead unprocessed but isnt a % value
        // numeral processed obj value processes parsed strings
        if (numeral(value)._value === 0 && Number(value) !== 0 && value.indexOf('%') === -1) {
            return value;
        }
        value = numeral().unformat(value);
        value = value * scaler;
