...
export class ChartDefaults {
    ...
    getDefaults(): any {
        ...
        let defaults = {
            ...
            colors: ['#003f11', '#0b9a33', '#94c11c', '#115622', '#d60229', '#e6ede1', '#f4f4f4', '#c4daa4', '#636362', '#000000', '#ffffff', '#69261b', '#ff5050'],
            credits: {
                enabled: false
            },
            xAxis: {
    ...
    getStackedColumnChartDefaults(numberFormat ? : string, totalFormat ? : string) {
        let chartData = {
            chart: {
                type: 'column'
            },
            ...
            yAxis: {
                ...
                stackLabels: {
                    ...
                    formatter: function() {
                        if(this.options.arrTotals[this.x] != undefined && this.options.arrTotals[this.x] != null && this.options.arrTotals[this.x] !== ""){            
                            if (totalFormat) {
                                return numberFormatting.format(this.options.arrTotals[this.x], totalFormat);
                            } else {
                                return this.options.arrTotals[this.x]
                            }
                        }
                        else{
                            return null;
    ...
    getBubbleChartDefaults(numberFormatX ? : string, numberFormatY ? : string, numberFormatZ ? : string, xLabel ? : string, yLabel ? : string, zLabel ? : string) {
        let chartData = {
            chart: {
                type: 'bubble'
            ...
            xAxis: {
                ...
                labels: {
                    formatter: function() {
                        let value = this.value || this.x;
                        if (numberFormatX) {
                            return numberFormatting.format(value, numberFormatX);
                        } else {
                            return value;
                        }
                    }
            ...
            yAxis: {
                ...
                labels: {
                    formatter: function() {
                        let value = this.value || this.y;
                        if (numberFormatY) {
                            return numberFormatting.format(value, numberFormatY);
                        } else {
                            return value;
                        }
                    }
            ...
            tooltip: { 
                useHTML: true, 
                // pointFormat: "UpGPal lift : {{point.x | }*100}% </br> ROI : {point.y} </br> Volume : {point.z} ",
                pointFormatter: function(){
                    var x = xLabel,y = yLabel,z = zLabel,xValue,yValue,zValue = this.z;
                    if (numberFormatX) {
                        xValue = numberFormatting.format(this.x, numberFormatX);
                    } else {
                        xValue = this.x;
                    }

                    if (numberFormatY) {
                        yValue = numberFormatting.format(this.y, numberFormatY);
                    } else {
                        yValue = this.y;
                    }
                    if (numberFormatZ) {
                        zValue = numberFormatting.format(this.z, numberFormatZ);
                    } else {
                        zValue = this.z;
                    }
                    var xPart = x != "" ? x + ":" + xValue +  "</br>" : "";
                    var yPart = y != "" ? y + ":" + yValue +  "</br>" : "";
                    var zPart = z != "" ? z + ":" + zValue +  "</br>" : "";
                    var str = xPart + yPart + zPart;
                    return str;
                },
                followPointer: true 
            },
            ...
            series: [{
                ...
                dataLabels: {
                    ...
                    formatter: function() {
                        if (numberFormatY) {
                            return numberFormatting.format(this.y, numberFormatY);
                        } else {
                            return this.y;
                        }
                    },
    ...
    getStackedBarChartDefaults(numberFormat ? : string) {
        let chartData = {
            chart: {
                type: 'bar'
            },
    ...
    getScatterChartDefaults(numberFormat ? : string) {
        let chartData = {
            chart: {
                type: 'scatter'
            },
    ...
    getDefaultsFor(chartType: string, numberFormat ? : string): any {
        switch (chartType) {
            ...
            case CHART_TYPE.BUBBLE:
                return this.getBubbleChartDefaults(numberFormat);
            case CHART_TYPE.STACKER_BAR:
                return this.getStackedBarChartDefaults(numberFormat);
            case CHART_TYPE.SCATTER:
            return this.getScatterChartDefaults(numberFormat);
