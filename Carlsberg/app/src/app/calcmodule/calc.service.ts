...
export class CalcService {
    ...
    private localStorageKey: string = "modelstate:CarlsbergBa1";
    ...
    setValue(rangeRef: string, value: any): Promise<any> {
        ...
        return self.api.setValue(rangeRef, value)
            ...
            if (emitEventOnComplete !== false) {
                self.saveStateToStorage();
                self.emitter.emit();
            }
            console.log("Model updated " + (timeLapsed/1000));
        });
            ...
            this.modelName = _model.modelName || this.modelName;
                    ...
                    new jsCalc({
                        ...
                        loadCallback: function() {
                            ...
                            self.forceRecalculate(resolve, calcApi);
                            // resolve(calcApi);
                        },
    ...
    loadModelData(modelData?:string): Promise<any> {
    ...
    appendDataToModel(stateOb:any): Promise<any> {
        return this.api.appendDataToModel(stateOb).then(() => {
            this.saveStateToStorage();
    ...
    getModelDefaults(arrRefs:Array<string>): Promise<any> {
        return new Promise((resolve, reject) => {
            let dictionary = new Collections.Dictionary<string, string>();
            this.buildModel().then((calcApi) => {
                // calcApi.getValue()
    ...
    forceRecalculate(resolve, calcApi) {
        // return resolve(calcApi);
        calcApi.getBook().recalculate();
        let startTime = window.performance.now(),
            subscription: any;
        calcApi.addCalculationCallback(function() {
            let endTime = window.performance.now();
            console.log(" model recalculated successfully! : start time: " + startTime + " - endTime" + endTime);
            subscription.unsubscribe();
            resolve(calcApi);
        });
        let workbook = calcApi.getBook(),
            maxCalcQueueLength = workbook.calculationQueue.length,
            totalCalculationsDone = 0,
            calcQueueLength = 0;

        return subscription  = Observable.interval(200)
            .subscribe(() => {

                if (workbook.calculationPending()) {
                    calcQueueLength = workbook.calculationQueue.length
                    maxCalcQueueLength = Math.max(maxCalcQueueLength, calcQueueLength);
                    totalCalculationsDone = maxCalcQueueLength - calcQueueLength;
                    this.progressCallBacks.forEach((cb) => {
                        let progressOb = {
                            numComplete: totalCalculationsDone, 
                            numTotal: maxCalcQueueLength
                        };
                        cb(progressOb);
                    });
                }
    ...
    exportData(isSpecificExport?:boolean, specificRangeNames?:any) {
        this.exportDecisions(isSpecificExport, specificRangeNames);
    }

    private exportDecisions(isSpecificExport?:boolean, specificRangeNames?:any) {
        let self = this;
        ...
        //     });
        // }
        var modelState = this.getAllInputsState(isSpecificExport, specificRangeNames);
        var teamNo = this.getValue("xxIndexTeam");
        var teamYear = this.getValue("xxRound");
        var fileNamePrefix = "CarlsbergVMdecT";
        var fileName = fileNamePrefix + teamNo + "R" + teamYear + ".csv";
        var delim = ',';
        self.fileSaver.fileSaveAs(fileName, modelState, delim);
    }

    getAllInputsState(isSpecificExport?:boolean, specificRangeNames?:any) {
        var pattern = new RegExp('^tl[I|i]nput.+$'),
            rangeNames,
            THOUSAND = 1000,
            out = {};

        rangeNames = isSpecificExport && specificRangeNames.length > 0 ? specificRangeNames : this.api.getNames(pattern);
